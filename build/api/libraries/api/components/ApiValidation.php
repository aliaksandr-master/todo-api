...
class ApiValidation extends ApiComponent {

    public function apply ($value, $ruleName, array $params = array(), $contextName = null) {
		$result = $this->api->context->applyValidationRule($value, $ruleName, $params, $contextName);
    ...
    function validate ($fieldName, $value, array $validation, $setError = false) {
        ...
        if($required && !$this->api->validation->apply($value, 'required')){
            if ($setError) {
        ...
        if (!$error && isset($value) && (strlen((string)$value) || $value === false)) {
            foreach ($rules as $rule) {
                ...
                if (!$this->api->validation->apply($value, $ruleName, $ruleParams, $fieldName)) {
                    if ($setError) {
    ...
    private function rule_need ($value, array $params = array(), $name = null) {
        $existFiled = $params[0];
        return $this->rule_required($this->api->input->get($existFiled), array($existFiled), $name);
    }
